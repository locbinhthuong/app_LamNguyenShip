...
export default function AlertModal({ isOpen, title, message, onConfirm, confirmText = "Đóng", isDestructive = false }) {
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-sm rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="p-6">
          <div className={`mb-4 inline-flex h-12 w-12 items-center justify-center rounded-full ${isDestructive ? 'bg-red-50 text-red-500' : 'bg-orange-50 text-orange-500'}`}>
