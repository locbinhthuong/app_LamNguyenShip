...
import { Clock, PackageCheck, Truck, CheckCircle, XCircle, TrendingUp, BarChart2, Package, DollarSign } from 'lucide-react';
...
const ShopActivity = () => {
  ...
  const [mainTab, setMainTab] = useState('statistics'); // 'statistics' or 'history'
  ...
  return (
    <div className="flex flex-col min-h-screen w-full bg-gray-50 font-sans overflow-hidden">
      ...
      {/* HEADER TABS CHÍNH */}
      <div className="shrink-0 bg-white pt-4 safe-pt relative z-40 border-b border-gray-200 shadow-sm">
        <div className="flex px-4">
          <button
            onClick={() => setMainTab('statistics')}
            className={`flex-1 py-3 text-sm font-bold flex justify-center items-center gap-2 border-b-2 transition-colors ${mainTab === 'statistics' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-400'}`}
          >
            <BarChart2 size={18} /> Thống Kê
          </button>
          <button
            onClick={() => setMainTab('history')}
            className={`flex-1 py-3 text-sm font-bold flex justify-center items-center gap-2 border-b-2 transition-colors ${mainTab === 'history' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-400'}`}
          >
            <Clock size={18} /> Lịch Sử Đơn
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden flex flex-col relative">
        ...
        {mainTab === 'statistics' && (
          <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Thống Kê Doanh Thu</h2>
            
            {/* Bộ lọc thống kê */}
            <div className="bg-white rounded-xl p-1.5 flex gap-1 mb-6 shadow-sm border border-gray-100 max-w-sm">
            ...
            {/* Biểu đồ */}
            <div className="bg-white rounded-3xl p-4 md:p-6 shadow-sm border border-gray-100 mb-20">
              ...
              <div className="w-full h-full shrink-0 snap-center overflow-y-auto p-4" style={{ minWidth: '100%' }}>
                {loading ? (
                  ...
                  <div className="text-center text-gray-500 mt-10 flex flex-col items-center">
                    ...
                    <button onClick={() => navigate('/shop/book/delivery')} className="mt-4 bg-blue-100 text-blue-600 px-4 py-2 rounded-full font-bold">Tạo đơn mới</button>
                  </div>
                ) : (
